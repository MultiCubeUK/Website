                    ...
                    if (($Session->CheckPermission('Plugins.OnlineNow.ViewHidden')) || ($User->Invisible < 1)) {
                     ...
                     //  echo UserPhoto($User, array('ImageClass' => 'onimage'));
                       $UserPhoto = UserPhoto($User, array('ImageClass' => 'onimage'));
                        ...
                        echo $UserPhoto;
                       $Href = Url(UserUrl($User));
                       echo '<div class="Time">'.'<a class="'.'Popup Name'.'" title="'.'Post to Activity'.'" href="'.$Href.'"'.'>'.htmlspecialchars($User->Name).'</a>'.'<a class="'.'Popup Time'.'" href="'.$PM.htmlspecialchars($User->Name).'" title="'.'Send PM'.'"'.'>'.Gdn_Format::Date($User->Timestamp).'</a>'.'</div>';
